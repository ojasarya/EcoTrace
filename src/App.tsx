...
function Donut({ items = sources, selected, onSelect }: { items?: typeof sources; selected?: string; onSelect?: (name: string) => void }) {
...
function Reports() {
  ...
  useEffect(() => {
    fetchFactories().then((factories) => factories[0] && fetchDashboard(factories[0].id)).then((dashboard) => dashboard && setCalculationId(dashboard.calculation_id)).catch(() => setCalculationId(null));
  }, []);
...
function Login() {
  ...
  return <div className="landing"><main className="landing-section" style={{ maxWidth: 520, margin: "0 auto" }}><div className="center-heading"><div className="eyebrow">EcoTrace workspace</div><h1>{isRegistering ? "Create your account" : "Sign in to your factory"}</h1><p>Use your account to access owned factory data and live analysis.</p></div><Card><form onSubmit={submit} className="input-grid"><label>Email<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} required autoComplete="email" /></label><label>Password<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} required autoComplete={isRegistering ? "new-password" : "current-password"} minLength={8} /></label>{error && <p role="alert">{error}</p>}<Button type="submit">{loading ? "Please wait..." : isRegistering ? "Create account" : "Sign in"}</Button></form></Card><p style={{ textAlign: "center" }}><button className="text-link" onClick={() => { setIsRegistering(!isRegistering); setError(null); }}>{isRegistering ? "Already have an account? Sign in" : "New to EcoTrace? Create an account"}</button></p><p style={{ textAlign: "center" }}><Link to="/">Back to home</Link></p></main></div>;
}
